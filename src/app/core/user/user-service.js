// TODO needs to be rewritten, implementation is pending completion of
// server side authentication mechanism (current implementation assumes
// koast-angular compatible api)
//
//
//
'use strict';

angular.module('koastAdminApp.core.user.user-service', []) //Stub for login-controller
  .factory('user', function($q) {
    return {
      login: R.always($q.when()),
      isAuthenticated: R.always(true)
    };
  });
...
//    'koast'
//  ])
//    .service('user', function (koastAdmin, $http, $timeout, _koastLogger) {
...
//        logout: function () {
//            cancelRefresh();
//            return makeBackupApiCall('logout').then(handleDestroyAuth);
//        },
//        refreshToken: function () {
//            log.debug('Refreshing koast admin auth token');
//            return makeBackupApiCall('refreshToken').then(handleSuccessAuth, function () {
